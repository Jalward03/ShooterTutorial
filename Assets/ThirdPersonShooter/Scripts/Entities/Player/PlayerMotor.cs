...
namespace ThirdPersonShooter.Entities.Player
{
	public class PlayerMotor : MonoBehaviour
	{
		...
		[SerializeField, Range(0, 1)] private float aligmentDamping = .1f;
		...
		private void FixedUpdate()
		{
			...
			if(Mathf.Approximately(input.normalized.magnitude, 1))
				render.forward = Vector3.SmoothDamp(render.forward, camera.forward, ref dampingVelocity, aligmentDamping);
		}
